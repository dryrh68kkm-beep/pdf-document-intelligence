...
Pure Python library (`process_document()`), no HTTP server, no
persistence, no multi-document state. The only UI was one static HTML
file (`frontend/review.html`) that a build script generated by embedding
one document's JSON + the source PDF (base64) directly into the page —
one file per review session, not an application.
...
- `state.js` — central store + pub/sub. All views re-render from this on
  any state change.
- `components/productTable.js` — manual virtual scroll: renders only the
  rows in the visible window (+buffer), not the full DOM, so the table
  stays smooth regardless of row count.
