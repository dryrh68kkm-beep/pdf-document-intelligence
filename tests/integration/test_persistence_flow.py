"""End-to-end persistence flow using a runtime-generated synthetic PDF.

No real/company document is required: upload -> process -> dashboard -> edit
-> restart -> duplicate detection remains covered with fictional data only.
"""
...
def test_upload_edit_restart_flow(tmp_path):
    ...
    sample = make_bigc_pdf(tmp_path / "synthetic-bigc.pdf")

    with sample.open("rb") as f:
        res = client.post("/api/documents", files={"file": ("synthetic.pdf", f, "application/pdf")})
    ...
    patch_res = client.patch(
        ...
        json={"field": "sku_qty", "value": old_sku_qty + 50, "reason": "synthetic verification"},
    )
    ...
    with sample.open("rb") as f:
        dup_res = client.post("/api/documents", files={"file": ("synthetic.pdf", f, "application/pdf")})
...
def test_local_master_learning_flow():
    ...
    add_res = client.post(
        "/api/master/local",
        json={"barcode": "9990000012345", "productName": "Synthetic Confirmed Product", "department": "SYNTHETIC"},
    )
...
def test_health_endpoint_supports_missing_external_master():
    store.reset_for_tests()
    client = TestClient(app)
