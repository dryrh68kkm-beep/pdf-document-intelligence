...
def test_valid_article_format_remains_high_confidence():
    ...
    assert "INVALID_ARTICLE_FORMAT" not in field.validation_flags
...
def test_valid_internal_barcode_is_accepted():
    ...
    assert field.review_required is False
    assert "INVALID_BARCODE_FORMAT" not in field.validation_flags
...
def _field(name: str, value: str) -> FieldValue:
    return FieldValue(
        ...
        raw_value=value,
        value=value,
        type="string" if name == "name" else "code",
        bbox=_bbox(),
        source="pdf_text",
        confidence=0.99,
    )
